...
import (
	...
	"github.com/magenx/hek3ster/internal/cluster"
...
var upgradeCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// Load configuration first to get k3s version
		loader, err := config.NewLoader(upgradeConfigPath, upgradeNewK3sVersion, upgradeForce)
		if err != nil {
			return fmt.Errorf("failed to load configuration: %w", err)
		}

		// Validate configuration for upgrade
		if err := loader.Validate("upgrade"); err != nil {
			if loader.HasErrors() {
				loader.PrintErrors()
			}
			return err
