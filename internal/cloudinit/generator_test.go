...
func TestGenerateWithLocalFirewall(t *testing.T) {
	config := &Config{
		SSHPort:                  2222,
		Packages:                 []string{},
		UseLocalFirewall:         true,
		HetznerToken:             "test-token",
		HetznerIPsQueryServerURL: "http://test.example.com",
		ClusterCIDR:              "10.42.0.0/16",
		ServiceCIDR:              "10.43.0.0/16",
		AllowedNetworksSSH:       []string{"0.0.0.0/0"},
		AllowedNetworksAPI:       []string{"0.0.0.0/0"},
	}
	...
	if !strings.Contains(cloudInit, "#cloud-config") {
		t.Error("Generated cloud-init doesn't contain #cloud-config directive")
	}

	if !strings.Contains(cloudInit, "/usr/local/bin/firewall.sh") {
		t.Error("Generated cloud-init doesn't contain firewall.sh file")
	}

	if !strings.Contains(cloudInit, "/etc/systemd/system/firewall.service") {
		t.Error("Generated cloud-init doesn't contain firewall.service file")
	}

	if !strings.Contains(cloudInit, "/usr/local/bin/firewall-status") {
		t.Error("Generated cloud-init doesn't contain firewall-status file")
	}

	if !strings.Contains(cloudInit, "/etc/allowed-networks-kubernetes-api.conf") {
		t.Error("Generated cloud-init doesn't contain allowed-networks-kubernetes-api.conf file")
	}

	if !strings.Contains(cloudInit, "/etc/allowed-networks-ssh.conf") {
		t.Error("Generated cloud-init doesn't contain allowed-networks-ssh.conf file")
	}

	if !strings.Contains(cloudInit, "/usr/local/bin/firewall.sh setup") {
		t.Error("Generated cloud-init doesn't contain firewall setup command")
	}

	if !strings.Contains(cloudInit, "systemctl daemon-reload && systemctl enable --now firewall.service") {
		t.Error("Generated cloud-init doesn't contain firewall service enable command")
	}

	t.Logf("Generated cloud-init with firewall:\n%s", cloudInit)
}
...
func TestGenerateWithoutLocalFirewall(t *testing.T) {
	...
	if strings.Contains(cloudInit, "/usr/local/bin/firewall.sh") {
		t.Error("Generated cloud-init should not contain firewall.sh file when UseLocalFirewall is false")
	}

	if strings.Contains(cloudInit, "/usr/local/bin/firewall.sh setup") {
		t.Error("Generated cloud-init should not contain firewall setup command when UseLocalFirewall is false")
	}
}
