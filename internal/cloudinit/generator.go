...
//go:embed templates/firewall/firewall.sh
var firewallScript string

//go:embed templates/firewall/firewall.service
var firewallService string

//go:embed templates/firewall/firewall_status.sh
var firewallStatusScript string

//go:embed templates/nat_gateway_cloud_init.yaml
var natGatewayCloudInitTemplate string

//go:embed templates/k3s/install_first_master.sh
var k3sInstallFirstMasterTemplate string

//go:embed templates/k3s/install_additional_master.sh
var k3sInstallAdditionalMasterTemplate string
...
type Config struct {
	...
	HetznerToken              string
	HetznerIPsQueryServerURL  string
	ClusterCIDR               string
	ServiceCIDR               string
...
func (g *Generator) Generate() (string, error) {
	...
	// Generate firewall files section
	firewallFiles, err := g.generateFirewallFiles()
	if err != nil {
		return "", fmt.Errorf("failed to generate firewall files: %w", err)
	}
...
func (g *Generator) generatePostCreateCommandsStr() string {
	...
	// Add firewall setup commands if local firewall is enabled
	if g.config.UseLocalFirewall {
		allCommands = append(allCommands,
			"/usr/local/bin/firewall.sh setup",
			"systemctl daemon-reload && systemctl enable --now firewall.service",
		)
	}
...
func (g *Generator) formatAdditionalCommands(commands []string) []string {
	...
	for _, cmd := range commands {
		if strings.Contains(cmd, "\n") {
			...
			formatted = append(formatted, multilineStr)
		} else {
...
// generateFirewallFiles generates the firewall configuration files section
func (g *Generator) generateFirewallFiles() (string, error) {
	// Skip if local firewall is not enabled
	if !g.config.UseLocalFirewall {
		return "", nil
	}

	// Render firewall script with configuration
	firewallScriptRendered, err := g.renderFirewallScript()
	if err != nil {
		return "", fmt.Errorf("failed to render firewall script: %w", err)
	}

	// Render firewall status script
	firewallStatusRendered, err := g.renderFirewallStatusScript()
	if err != nil {
		return "", fmt.Errorf("failed to render firewall status script: %w", err)
	}

	// Encode firewall files
	firewallScriptEncoded := g.encodeAndFormat(firewallScriptRendered)
	firewallServiceEncoded := g.encodeAndFormat(firewallService)
	firewallStatusEncoded := g.encodeAndFormat(firewallStatusRendered)

	// Generate allowed networks files content
	sshNetworksContent := strings.Join(g.config.AllowedNetworksSSH, "\n")
	apiNetworksContent := strings.Join(g.config.AllowedNetworksAPI, "\n")
	sshNetworksEncoded := g.encodeAndFormat(sshNetworksContent)
	apiNetworksEncoded := g.encodeAndFormat(apiNetworksContent)

	// Define file paths as constants
	const (
		apiNetworksPath     = "/etc/allowed-networks-kubernetes-api.conf"
		sshNetworksPath     = "/etc/allowed-networks-ssh.conf"
		firewallScriptPath  = "/usr/local/bin/firewall.sh"
		firewallServicePath = "/etc/systemd/system/firewall.service"
		firewallStatusPath  = "/usr/local/bin/firewall-status"
	)

	// Build the firewall files section
	files := []string{
		fmt.Sprintf("- content: %s\n  path: %s\n  encoding: gzip+base64", apiNetworksEncoded, apiNetworksPath),
		fmt.Sprintf("- content: %s\n  path: %s\n  encoding: gzip+base64", sshNetworksEncoded, sshNetworksPath),
		fmt.Sprintf("- path: %s\n  permissions: '0755'\n  content: %s\n  encoding: gzip+base64", firewallScriptPath, firewallScriptEncoded),
		fmt.Sprintf("- path: %s\n  content: %s\n  encoding: gzip+base64", firewallServicePath, firewallServiceEncoded),
		fmt.Sprintf("- path: %s\n  permissions: '0755'\n  content: %s\n  encoding: gzip+base64", firewallStatusPath, firewallStatusEncoded),
	}

	return strings.Join(files, "\n"), nil
}

// renderFirewallScript renders the firewall script template with configuration
func (g *Generator) renderFirewallScript() (string, error) {
	tmpl, err := template.New("firewall.sh").Parse(firewallScript)
	if err != nil {
		return "", err
	}
	...
	var buf bytes.Buffer
	data := map[string]interface{}{
		"hetzner_token":                g.config.HetznerToken,
		"hetzner_ips_query_server_url": g.config.HetznerIPsQueryServerURL,
		"ssh_port":                     g.config.SSHPort,
		"cluster_cidr":                 g.config.ClusterCIDR,
		"service_cidr":                 g.config.ServiceCIDR,
	}
	if err := tmpl.Execute(&buf, data); err != nil {
		return "", err
	}
...
// renderFirewallStatusScript renders the firewall status script template
func (g *Generator) renderFirewallStatusScript() (string, error) {
	tmpl, err := template.New("firewall_status.sh").Parse(firewallStatusScript)
	if err != nil {
		return "", err
	}
...
// GenerateK3sInstallFirstMasterCommand generates k3s installation command for first master
func GenerateK3sInstallFirstMasterCommand(k3sVersion, k3sToken, baseArgs string) (string, error) {
	tmpl, err := template.New("install_first_master.sh").Parse(k3sInstallFirstMasterTemplate)
	if err != nil {
		return "", fmt.Errorf("failed to parse k3s first master install template: %w", err)
	}
	...
	if err := tmpl.Execute(&buf, data); err != nil {
		return "", fmt.Errorf("failed to execute k3s first master install template: %w", err)
	}

	return strings.TrimSpace(buf.String()), nil
}

// GenerateK3sInstallAdditionalMasterCommand generates k3s installation command for additional masters
func GenerateK3sInstallAdditionalMasterCommand(k3sVersion, k3sToken, baseArgs string) (string, error) {
	tmpl, err := template.New("install_additional_master.sh").Parse(k3sInstallAdditionalMasterTemplate)
	if err != nil {
		return "", fmt.Errorf("failed to parse k3s additional master install template: %w", err)
	}
	...
	if err := tmpl.Execute(&buf, data); err != nil {
		return "", fmt.Errorf("failed to execute k3s additional master install template: %w", err)
	}
