# GitHub Copilot Instructions for Hek3ster

## Project Overview
Hek3ster is a Kubernetes cluster management tool written in Go that automates cluster creation, management, and operations on Hetzner Cloud infrastructure. This project uses K3s (lightweight Kubernetes) and provides CLI commands for cluster lifecycle management.
...
./dist/hek3ster create --config test-cluster.yaml --dry-run

# Test run command
./dist/hek3ster run --config test-cluster.yaml --command "uptime"
...
1. Create command file in `cmd/hek3ster/commands/`
